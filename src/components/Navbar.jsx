  ...
  return (
    <nav className={`w-full fixed top-0 left-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-black/90 backdrop-blur-md' : 'bg-transparent'}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
        ...
        >
          <ul className="flex flex-col md:flex-row items-center md:space-x-6 space-y-8 md:space-y-0">
            {sections.map((section, idx) => (
              <li key={section}>
                <a
                  ...
                  className={`text-xl md:text-base px-4 py-2 text-white hover:text-yellow-500 transition-colors cursor-pointer ${
